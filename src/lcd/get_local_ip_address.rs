        ...
        Ok(output) => {
            ...
            if output_as_ascii.contains("100 (") {
                ...
                let output_as_a_vec_of_lines: Vec<&str> = // get the output as a vec of individual lines
                    output_as_ascii.split(|c: char| c == '\n').collect();
                    ...
                    match wifi_data_as_result {
                        Ok(wi_fi_data) => {
                            ...
                            match output2_as_result {
                                Ok(result_as_bytes) => {
                                    if result_as_bytes.stdout.len() > 0 {
                                            ...
                                            .contains("successfully activated with ")
                                        {
                                            ...
                                            match try_once_to_get_wifi_network_data() {
                                                ...
                                                Err(error_message) => {
                                                    return Err(format!( "Set Wi-Fi password seemingly sucessfully but could not get the IP addresss and got error {}", error_message).to_string());
                                                }
                                            }
                                        } else {
                                            return Err(
                                                "Tried to set Wi-FI SSID & password but failed"
                                                    .to_string(),
                                            );
                                        };
                                    } else if result_as_bytes.stderr.len() > 0 {
                                        ...
                                        return Err(format!(
                                            ...
                                            stderr_output[0..stderr_output.len() - 1].to_string()
                                        ));
                                    } else {
